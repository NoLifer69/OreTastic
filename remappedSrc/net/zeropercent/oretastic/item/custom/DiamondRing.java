...
public class DiamondRing extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if(user instanceof PlayerEntity){
            ...
            PlayerEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.SPEED, 1200, 1));
            user.getItemCooldownManager().set(ModItems.EMERALD_RING, 1200);
            user.getItemCooldownManager().set(ModItems.DIAMOND_RING, 1500);
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
            tooltip.add(Text.literal("Gives You The Speed Effect For 60 Seconds").formatted(Formatting.AQUA));
